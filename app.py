                    ...
                    if new_status == "Resolved" and note.strip():
                        ...
                        if db:
                            ...
                            if not existing.data:
                                ...
                                st.success("✅ Updated & knowledge base refreshed!")
                    else:
                        st.success("Updated!")
                    st.rerun()
                except Exception as e:
...
def page_analytics():
    ...
    # ── Row 2: Tickets per day + Status pie ───────────────────────────────────
...
def page_knowledge_gap():
    ...
    # ── All failed queries list ───────────────────────────────────────────────
    st.markdown("### 📋 All Unanswered Questions")
    st.caption("Add answers to these in your PDF or resolve related tickets to fill these gaps.")
    ...
    # ── Clear button ──────────────────────────────────────────────────────────
    st.markdown("---")
    if st.button("🗑️ Clear All Failed Queries (after fixing PDF)"):
        try:
            db.table("failed_queries").delete().neq("id", 0).execute()
            st.success("Cleared! Re-upload your updated PDF and restart the app.")
            st.rerun()
        except Exception as e:
            st.error(str(e))
