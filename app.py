...
def _init():
    for k, v in {
        ...
        "ap_escalation_msgs": [],
        "hub_type": None, "hub_inc_show_form": False,
        "hub_inc_kb_status": None, "hub_doc_show_form": False,
    }.items():
...
.hub-choice-box {
    background:#faf7f2; border:1.5px solid #d4c9bc; border-radius:8px;
    padding:28px 24px; text-align:center; cursor:pointer;
    transition:border-color .18s, box-shadow .18s;
}
.hub-choice-box:hover { border-color:#8b3a2a; box-shadow:0 2px 12px rgba(139,58,42,.12); }
.hub-info-banner {
    background:#fffbeb; border:1px solid #fcd34d; border-left:3px solid #f59e0b;
    border-radius:3px; padding:14px 18px; font-size:20px; color:#92400e; margin:10px 0;
}
</style>
"""


# ── Ticket Hub (Tab 1 — replaces old Submit tab) ──────────────────────────────

def _view_ticket_hub():
    from employee_portal import _hub_incident_flow, _hub_doc_approval_flow, _tab_my_tickets, _CSS as EP_CSS
    st.markdown(EP_CSS, unsafe_allow_html=True)

    st.markdown("### Choose Ticket Type")
    st.markdown(
        "<p style='color:#6b5f55;font-size:20px;font-family:EB Garamond,serif;'>"
        "Select the type of request you want to raise.</p>",
    ...
    with col_a:
        if st.button(
            "🚨  Incident Ticket\n\nReport a problem or bug. KB is searched first — ticket raised if no answer found.",
    ...
    with col_b:
        if st.button(
            "📄  Document Approval Ticket\n\nRequest document creation. Routed through the correct approval chain automatically.",
            key="hub_btn_doc",
            use_container_width=True,
        ):
...
def page_approval_pipeline():
    ...
    with hc: st.title("📋 Approval Pipeline")
    with rc:
...
# ── Submit tab (kept for internal use / pre-fill from doc validator) ──────────
